			...
			{show && (
				<>
					<div className=' text-white absolute top-[200px] w-full  flex flex-col gap-[20px]'>
						...
						{localOrGlobal === "local" ? (
							<LocalHiScores setMode={setMode} mode={mode} scores={scores} />
						) : (
...
function GlobalHiScores({
	setMode,
	mode
}: {
	setMode: (difficulty: "easy" | "medium" | "hard") => void
				...
				{scores ? (
					scores[mode].map((s, i) => (
						...
						>
							...
							<div className='text-right pr-[30px]'>
								{s.time !== 0 ? buildClock(s.time) : "-----"}
							</div>
						</div>
					))
				) : !error ? (
					...
					<div>
						There was an error fetching the HiScores. Please check that you're
